...
# load data ---------------------------------------------------------------
path <- here("raw-data", "cing-y.csv")
df <- read_csv(path, na = c("nan", "#DIV/0!", ""), 
               col_types = "fffiiifffii--ii-iiiiinnffff")

# recording ---------------------------------------------------------------
# filter out control (no sham surgery was performed) and unilateral cingulotomy
# cases, all observations that Sean has flagged as unusual, ta19 because his 
# cingulotomy was placed more posteriorly than planned in area A23


df_filt <- df %>% dplyr::filter(treatment %in% c("sham", "cingulotomy"),
                                is.na(notecodes), id != "ta19")
...
# added sex and twin status predictors
# the fit of the model has deteriorated & convergence issues has emerged
model_twin_sex <- lmer(percentphee ~ opcondition + treatment + sex + twingroup +
                                     pol_age1 + pol_age2 +
                                     ((pol_age1 + pol_age2)|twingroup/id), 
                                   data = df_filt_m,
                                   REML = FALSE, 
                                   control=lmerControl(optimizer="bobyqa")) 
summary(model_twin_sex)
